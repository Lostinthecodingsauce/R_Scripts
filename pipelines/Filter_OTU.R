...
transform_OTU <-data_sum_OTU%>%
  mutate_if(is.numeric, sqrt)%>% #Normalizing data (arcsin(sqrt(x)))
  mutate_if(is.numeric, asin)

#write csv
write_csv(transform_OTU, "CR_OTU_Clean_Transform.dat")

##transposed in JMP then put back in for data wrangling
transpose_otu <- read_csv("Transpose_CR_OTU_Clean_Transform.dat")%>%
...
write_csv(transpose_otu, "CR_OTU_WDF.dat")
